...
import importlib
import sys
import bigan.mydata_utilities as network
...
def create_logdir(weight, method, rd):
    """ Directory to save training logs, weights, biases, etc."""
    return "bigan/train_logs/mydata/{}/{}/{}".format(method, weight, rd)
...
def train_and_test(nb_epochs, weight, method, degree, random_seed):
    ...
    """
    logger = logging.getLogger("BiGAN.train.mydata.{}".format(method))

    # Placeholders
    input_pl = tf.placeholder(tf.float32, shape=[None,data.input_size, data.input_size,3], name="input")
    ...
    # Data
    train_set = data.MyData('train', batch_size=batch_size)
    test_set = data.MyData('test', batch_size=batch_size)
    ...
    with tf.name_scope('summary'):
        ...
        with tf.name_scope('image_summary'):
            tf.summary.image('reconstruct', reconstruct[:,:,:,::-1], 8, ['image'])
            tf.summary.image('input_images', input_pl[:,:,:,::-1], 8, ['image'])
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            for t,trainx in enumerate(train_set):
                display_progression_epoch(t, train_set.num_batches)
                trainx_copy = trainx.copy()

                # train discriminator
                feed_dict = {input_pl: trainx,
                             is_training_pl: True,
                             learning_rate: lr}
                ...
                feed_dict = {input_pl: trainx_copy,
                             is_training_pl: True,
                ...
                _, _, le, lg, sm = sess.run([train_gen_op,
                                             ...
                                             sum_op_gen],
                                            feed_dict=feed_dict)
                # train generator and encoder twice
                feed_dict = {input_pl: trainx_copy,
                             is_training_pl: True,
                             learning_rate: lr*100}
                _, _, le, lg, sm = sess.run([train_gen_op,
                ...
                if t % FREQ_PRINT == 0:  # inspect reconstruction
                    sm = sess.run(sum_op_im, feed_dict={input_pl: trainx, is_training_pl: False})
                    writer.add_summary(sm, train_batch)

                train_batch += 1

            train_loss_gen /= train_set.num_batches
            train_loss_enc /= train_set.num_batches
            train_loss_dis /= train_set.num_batches
...
def run(nb_epochs, weight, method, degree, label=None, random_seed=42):
